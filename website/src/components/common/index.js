...
export const DownloadIcon = ({ disabled }) => {
    ...
    return (
        <svg
            className={styles['downloadIcon' + disableModifier]}
            height="14"
            viewBox="0 0 14 14"
            width="14"
...
export const DownloadTooltip = ({ children, disabled }) => (
    <Tooltip disabled={disabled} html={<span>Please select latest schema version to download templates</span>}>
        {children}
    </Tooltip>
);

export const Display = ({ children, visible }) => <div style={{ display: visible ? 'block' : 'none' }}>{children}</div>;
